...
select emp_name as Name,job_id as Job,salary,salary*100*12 "Increase Salary" from employees;

-- [3] H R 부서에서 예산 편성 문제로 급여 정보 보고서를 작성하려고 한다.
-- 사원정보(Employees) 테이블에서 급여가 $7,000~$10,000 범위 이외인 
-- 사람의 이름과 성(Name으로 별칭) 및 급여를 급여가 적은 순서로 출력하시오(75행).
select emp_name as Name, salary from employees
where salary< 7000 or salary >10000
order by salary
;

-- not between을 사용: 2번째 방법 
select emp_name as Name,salary from employees
...
select employee_id, emp_name as Name, department_id, salary,
...
select emp_name as Name,salary,salary+salary*nvl(commission_pct,0) as comm_salary,
commission_pct,
-- 1. case when then
...
-- 출력 시 머리글은 '그룹함수명' 별칭(alias) 처리하시오(11행).
select department_id, 
to_char(sum(salary),'$999,999,999') sum_sal,
to_char(round(avg(salary),2),'$999,999,999') avg_sal,
...
where job_id not like '%CLERK%'   -- 일반컬럼의 조건을 넣는 곳
group by job_id
having avg(salary) > 10000 -- 그룹컬럼의 조건을 넣는 곳
