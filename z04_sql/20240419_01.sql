...
select * from mem3;
select no,mdate2,tel,name,mdate from mem3; -- 순서바꿔서 출력 가능

select * from employees;
-- 사원번호(e id), 사원이름(e name) ,급여(salary, 입사일지(hire_date)

select employee_id,emp_name,salary,hire_date from employees;

desc employees; -- desc : 타입 볼 때
...
-- 번호, 이름, 국어점수, 국어점수-20, 평균, 평균-20을 한 평균
